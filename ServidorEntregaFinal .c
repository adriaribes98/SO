...
void * procesar(void *socket){
	...
	while (acabar==0){
		...
		if(opcion==1){//OPCION 1 = AÑADIR UN JUGADOR
			...
			int d = damesocket(consulta,&misconectados);
			printf("El valor del socket es: %d\n ", d);
			sprintf(consulta,"9/%d/", mischats.num);
			//Enviamos la respuesta
			if (d == -1){
				write (sock_conn,"9/Ese usuario no esta conectado o no existe", strlen("9/Ese usuario no esta conectado o no existe"));
			}
			else
		...
		if(opcion==12){
			p=strtok(NULL,"/");
			char nombre_invitador[100];
			strcpy (nombre_invitador, p);
			p=strtok(NULL,"/");
			char nombre_invitado[100];
			strcpy (nombre_invitado, p);
			int d1 = damesocket(nombre_invitador,&misconectados);
			int d2 = damesocket(nombre_invitado,&misconectados);
