...
TListachats mischats; //lista chats
...
int inserta(char nombre[30],int socket, TListaConectados*misconectados){//añadir un conectado a la lista
	...
	int damesocket (char nombre[20], TListaConectados*misconectados){
		//retorna nombre 0 si okey -1 error
		int encontrado=0;
		int i=0;
		while((i<misconectados->num)&&(encontrado == 0)){
			if((strcmp(misconectados->conectados[i].nombre,nombre)==0))
		...
		if(encontrado){
			printf("%d\n",misconectados->conectados[i].socket);
	...
	int damenombre (int socket, TListaConectados*misconectados, char resultado[20]){
		//retorna nombre 0 si okey -1 error
		int encontrado=0;
		int i=0;
		while((i<misconectados->num)&&(encontrado == 0)){
			if((strcmp(misconectados->conectados[i].socket,socket)==0))
				encontrado=1;
			else 
				i=i+1;
		}
		if(encontrado){
			printf("%s\n",misconectados->conectados[i].nombre);
			strcpy (resultado, misconectados->conectados[i].nombre);
			return (resultado, misconectados->conectados[i].nombre);
...
void * procesar(void *socket){
	...
	conn = mysql_real_connect (conn, "shiva2.upc.es","root", "mysql", "GameAdria",0, NULL, 0);
	if (conn==NULL) {
	...
	while (acabar==0){
		...
		if(opcion==1){//OPCION 1 = AÑADIR UN JUGADOR
		...
		else if(opcion==2){//OPCION 2 = INICIAR SESION
			p=strtok(NULL,"/");
			strcpy(consulta, p);
			char password[20];
			...
			//write (sock_conn,salida, strlen(salida));
			//si se selecciona algo se envia su correspondiente respuesta
			//printf ("Respuesta2.1: %s\n", salida);
			
		}
		
		else if (opcion==3){//OPCION 3 = Numero de veces que tengo exclusion
			sprintf(salida,"3/%d",contador);  
		}
		
		else if (opcion==4){ //OPCION 4 = DAME JUGADOR Y TE DARE LA EXPERIENCIA
		...
		else if (opcion==5){//OPCION 5 = DAME JUGADOR Y TE DARÉ SUS PUNTOS
			p=strtok(NULL,"/");
			strcpy(consulta, p);
			printf("Opcion: %d\n",opcion);
			char jug[30];
		...
		else if (opcion==6){//OPCION 6 = DAME PAIS Y TE DARA EL JUGADOR
			p=strtok(NULL,"/");
			strcpy(consulta, p);
			char pais[500];
		...
		else if(opcion==7){//OPCION 7: Lista de conectados
			char res[512];
			...
			dameconectados(resultado, &misconectados);
			sprintf(notificacion,"8/%s",resultado);
			printf ("Respuesta8: %s y conectados: %d\n", salida, misconectados.num);
	    ...
	    else if(opcion==0){//OPCION 0 = DESCONECTAR
			acabar=1;
			char notificacion[512];
			notificacion[0]='\0';
		    p=strtok(NULL,"/");
			...
			strcpy(nombreinvitado,"");
			
			p=strtok(NULL,"/");
			strcpy(nombreinvitado, p);
			printf("Nombre del invitado es: %s\n", nombreinvitado);
			printf("Nombre del que invita es: %s\n", usuario);
			
			int d = damesocket(nombreinvitado,&misconectados);
			printf("El valor del socket es: %d\n ", d);
			sprintf(consulta,"9/%d/%s/%s/", mischats.num, usuario, nombreinvitado);
			//Enviamos la respuesta
			if (d == -1){
				write (sock_conn,"24/Ese usuario no esta conectado o no existe", strlen("24/Ese usuario no esta conectado o no existe"));
			}
			else
				
			{
				//write(sock_conn,consulta, strlen(consulta));
				write (d,consulta, strlen(consulta));
				printf ("Respuesta: %s\n", consulta);
				mischats.chats[mischats.num].socket_invitado = d;
				mischats.chats[mischats.num].socket_invitador = sock_conn;	
				mischats.num ++;
		    }
			//si se selecciona algo se envia su correspondiente respuesta
		...
		if(opcion==10){
			...
			strcpy(socket,p);
			printf("%s \n", socket);
			damenombre(socket,&misconectados,resultado);
			printf("El valor de nombre es: %s\n", resultado);
			//Enviamos la respuesta
			write (sock_conn,salida, strlen(salida));
			//si se selecciona algo se envia su correspondiente respuesta
			printf ("Respuesta: 10/%s\n", salida);
		}
		if(opcion==11){
			p=strtok(NULL,"/");
			char frase[100];
			...
			sprintf(notificacion,"11/%s/%s/",frase, nombrejug);
			...
			for(j=0;j<misconectados.num;j++){
				...
				write(misconectados.conectados[j].socket,notificacion,strlen(notificacion));
				printf("enviado : %s por socket %d\n", notificacion , misconectados.conectados[j].socket);
			}
...
/*			strcpy (nombre_invitado, p);*/
			int d1 = mischats.chats[id].socket_invitador;
			int d2 = mischats.chats[id].socket_invitado;
			
			char notificacion[512];
			
			sprintf(notificacion,"12/%d/%d/",d1,d2);
