...
int inserta(char nombre[30],int socket, TListaConectados*misconectados){//añadir un conectado a la lista
	...
	int damesocket (char nombre[20], TListaConectados*misconectados, char resultado[20]){
		...
		if(encontrado){
			...
			//strcpy (resultado, misconectados->conectados[i].socket);
...
void * procesar(void *socket){
	...
	while (acabar==0){
		...
		if(opcion==9){
			...
			int d = damesocket(consulta,&misconectados,resultado);
			printf("El valor del socket es: %d\n ", d);
			sprintf(consulta,"9/%s",resultado);
			//Enviamos la respuesta
			if (d == -1){
